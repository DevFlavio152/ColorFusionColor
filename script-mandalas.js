...
// VARIÁVEL GLOBAL DE SELEÇÃO MÚLTIPLA
...
function renderFullscreenColors() {
    ...
    listaCores.forEach(cor => {
        ...
        btn.onclick = () => {
            ...
            if (selectedParts.size > 0 && typeof tocarSom === 'function') {
                tocarSom('pintar');
            }
...
function setupMandala1() {
    document.querySelectorAll('.mandala-part').forEach(part => {
        part.onclick = (e) => {
            ...
            // Alterna a seleção (Se já selecionado -> remove; senão -> adiciona)
            ...
            if (msgBox) {
                msgBox.innerText = selectedParts.size > 0 
                    ? `${selectedParts.size} parte(s) selecionada(s). Escolha uma cor!` 
                    : "Selecione uma ou mais partes da mandala.";
            }
        };
    });
}
...
function applyColor1(hex, grupo) {
    if(selectedParts.size === 0 || mandala1Finished) return;
    tocarSom('pintar'); // 👈 ADICIONE AQUI
    
    selectedParts.forEach(part => {
        part.classList.remove('selected');
        // Força a cor tanto via atributo SVG quanto via estilo inline com !important
        part.setAttribute('fill', hex);
        part.style.setProperty('fill', hex, 'important');
        paintedParts1.set(part.id, grupo);
    });

    // Limpa a seleção após pintar
    selectedParts.clear();

    const msgBox = document.getElementById('msg-box');
    if (msgBox) msgBox.innerText = "Partes pintadas! Selecione novas áreas se desejar.";
...
function validateMandala1() {
    ...
    if(erros.length === 0) {
        ...
        selectedParts.forEach(p => p.classList.remove('selected'));
        selectedParts.clear();
...
function setupMandala2() {
    document.querySelectorAll('.mandala-part-2').forEach(part => {
        part.onclick = () => {
            ...
            const msgBox = document.getElementById('msg-box-2');
            if (msgBox) {
                msgBox.innerText = selectedParts.size > 0 
                    ? `${selectedParts.size} parte(s) selecionada(s). Crie contraste!` 
                    : "Misture as cores! Selecione partes da mandala.";
            }
        };
    });
}
...
function applyColor2(hex, grupo) {
    if(selectedParts.size === 0 || mandala2Finished) return;
    tocarSom('pintar'); // 👈 ADICIONE AQUI
    ...
    selectedParts.forEach(part => {
        ...
        part.setAttribute('fill', hex);
        part.style.setProperty('fill', hex, 'important');
        paintedParts2.set(part.id, grupo);
    });
...
function resetPhase2() {
    paintedParts2.clear();
    selectedParts.clear();
    document.querySelectorAll('.mandala-part-2').forEach(p => {
    ...
    document.getElementById('msg-box-2').innerText = "Misture as cores!";
}
...
function runTutorialStep() {
    ...
    if (tutorialStep === 1) {
        ...
        typeTutorialText(`Para começar sua obra, ${actionWord} em uma ou mais partes da mandala, como esta área que destaquei.`);
    } 
